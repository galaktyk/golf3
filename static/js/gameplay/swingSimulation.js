...
export function createSwingSimulation(params) {
  ...
  const enqueueIncomingSwingSample = (quaternion, swingState, receivedAtTimeMs) => {
    ...
    pendingSwingSamples.push({
      ...
      linearSpeedMetersPerSecond: Number.isFinite(swingState.linearSpeedMetersPerSecond) ? swingState.linearSpeedMetersPerSecond : 0,
  ...
  const consumeSwingSamplesUpTo = (simulationTimeMs) => {
    ...
    while (pendingSwingSamples.length > 0 && pendingSwingSamples[0].receivedAtTimeMs <= simulationTimeMs) {
      ...
      simulatedSwingState.linearSpeedMetersPerSecond = sample.linearSpeedMetersPerSecond;
  ...
  const getIncomingClubHeadSpeedMetersPerSecond = (swingState = simulatedSwingState, referenceTimeMs = performance.now()) => {
    ...
    const angularSpeed = swingState.perpendicularAngularSpeedRadiansPerSecond * len * PHONE_ANGULAR_SPEED_TO_CLUB_HEAD_SPEED_GAIN;
    const linearSpeed = swingState.linearSpeedMetersPerSecond ?? 0;
    return 2 * linearSpeed + 0.2 * angularSpeed;
  };
  ...
  const reset = () => {
    ...
    simulatedSwingState.linearSpeedMetersPerSecond = 0;
