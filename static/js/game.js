...
import { resolveClubBallImpact, HIT_ZONE_HALF_WIDTH, HIT_ZONE_HALF_HEIGHT } from '/static/js/game/impact/clubImpact.js';
...
const practiceSwingBallColor = new THREE.Color('#22e0ee');
const PRACTICE_SWING_BALL_OPACITY = 1.0;
const PRACTICE_SWING_BALL_OUTLINE_SCALE = 1.6;
const ballMaterialVisualState = new WeakMap();
/** @type {Map<THREE.Mesh, THREE.Mesh>} Maps original ball mesh -> its practice outline mesh. */
const ballPracticeOutlines = new Map();
...
/**
 * Wireframe 1×1 m hit-zone plane shown at the ball position during the control
 * phase so players can see exactly where they need to swing through.
 */
const _hitZonePlaneNormal = new THREE.Vector3();
const _hitZonePlaneDefaultNormal = new THREE.Vector3(0, 0, 1);
const hitZoneDebugMesh = (() => {
  const geo = new THREE.PlaneGeometry(HIT_ZONE_HALF_WIDTH * 2, HIT_ZONE_HALF_HEIGHT * 2);
...
viewerScene.scene.add(hitZoneDebugMesh);
...
function animate() {
  ...
  // Update the hit-zone debug plane: position it at the ball and orient its normal
  // to match the character's current horizontal facing direction.
  const isControlPhase = playerState === 'control' && ballTele.phase === 'ready';
  hitZoneDebugMesh.visible = isControlPhase;
  if (isControlPhase) {
    hitZoneDebugMesh.position.copy(ballPhysics.getPosition());
    viewerScene.getCharacterForward(_hitZonePlaneNormal);
    _hitZonePlaneNormal.y = 0;
    if (_hitZonePlaneNormal.lengthSq() > 1e-8) {
      _hitZonePlaneNormal.normalize();
      hitZoneDebugMesh.quaternion.setFromUnitVectors(_hitZonePlaneDefaultNormal, _hitZonePlaneNormal);
    }
  }
...
function syncPracticeSwingBallVisualState() {
  ...
  // Remove all existing outline meshes before re-evaluating.
  for (const [, outlineMesh] of ballPracticeOutlines) {
    outlineMesh.parent?.remove(outlineMesh);
  }
  if (!practiceSwingMode) ballPracticeOutlines.clear();

  viewerScene.ballRoot.traverse((node) => {
    // Skip outline meshes injected by this function.
    if (!node.isMesh || !node.material || node.userData.isPracticeOutline) return;
    ...
    for (const material of mats) {
      ...
      material.transparent = practiceSwingMode ? (PRACTICE_SWING_BALL_OPACITY < 1) : vs.transparent;
    ...
    // Add a back-face outline mesh as a child to create a black border in practice mode.
    if (practiceSwingMode) {
      if (!ballPracticeOutlines.has(node)) {
        ...
        outlineMesh.scale.setScalar(PRACTICE_SWING_BALL_OUTLINE_SCALE);
        outlineMesh.userData.isPracticeOutline = true;
        ballPracticeOutlines.set(node, outlineMesh);
      }
      node.add(ballPracticeOutlines.get(node));
    }
  });
}
