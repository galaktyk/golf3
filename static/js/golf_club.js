...
const integratedLinearVelocity = new THREE.Vector3();

const SWING_SPEED_FOLLOW_RATE = 8;
const SWING_SPEED_DECAY_RATE = 4;
const LINEAR_SPEED_DRIFT_DECAY_RATE = 3;
...
let filteredLinearSpeedMetersPerSecond = 0;
let decayingLinearSpeedMetersPerSecond = 0;
...
if (motionEventName) {
  window.addEventListener(motionEventName, (event) => {
    ...
    // Integrate linear acceleration to estimate phone translational speed.
    ...
    if (Number.isFinite(ax) && Number.isFinite(ay) && Number.isFinite(az)) {
      const driftDecayMultiplier = Math.exp(-LINEAR_SPEED_DRIFT_DECAY_RATE * deltaSeconds);
      integratedLinearVelocity.multiplyScalar(driftDecayMultiplier);
      integratedLinearVelocity.x += ax * deltaSeconds;
      integratedLinearVelocity.y += ay * deltaSeconds;
      integratedLinearVelocity.z += az * deltaSeconds;
    }
    const instantaneousLinearSpeedMetersPerSecond = integratedLinearVelocity.length();
    filteredLinearSpeedMetersPerSecond = THREE.MathUtils.lerp(
      filteredLinearSpeedMetersPerSecond,
      instantaneousLinearSpeedMetersPerSecond,
      followAlpha,
    );
    decayingLinearSpeedMetersPerSecond = Math.max(
      filteredLinearSpeedMetersPerSecond,
      decayingLinearSpeedMetersPerSecond * decayMultiplier,
    );
...
setInterval(() => {
  ...
  controllerSession.sendSwingPacket(encodeSwingStatePacket({
    ...
    linearSpeedMetersPerSecond: decayingLinearSpeedMetersPerSecond,
