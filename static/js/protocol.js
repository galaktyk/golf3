...
export const SWING_PACKET_VERSION = 2;
export const SWING_PACKET_KIND = 1;
export const SWING_PACKET_SIZE_BYTES = 18;
...
export function encodeSwingStatePacket({
  ...
  view.setUint16(16, encodeHundredths(linearSpeedMetersPerSecond), true);
...
export function decodeSwingStatePacket(buffer, targetQuaternion, targetState = {}) {
  ...
  const isV1 = buffer.byteLength === 16;
  const isV2 = buffer.byteLength === SWING_PACKET_SIZE_BYTES;
  if (!isV1 && !isV2) return null;

  const view = new DataView(buffer);
  const version = view.getUint8(0);
  const kind = view.getUint8(1);
  if (kind !== SWING_PACKET_KIND) return null;
  if (version !== 1 && version !== SWING_PACKET_VERSION) return null;
  ...
  targetState.linearSpeedMetersPerSecond = isV2 ? decodeHundredths(view.getUint16(16, true)) : 0;
  return targetState;
}
